...
export default function Navbar() {
  ...
  return (
    <nav className="px-4 border-b">
      <div className="max-w-screen-xl py-4 mx-auto grid grid-cols-[auto_1fr] lg:grid-cols-[auto_1fr_auto] gap-4">
        <h1 className='text-2xl'>BookReveal</h1>

        <div className="flex items-center justify-end lg:order-1 gap-4">
          ...
          <button className="lg:hidden border p-0.5 text-2xl hover:scale-95" onClick={() => setShowLinks(!showLinks)}>
            <List />
          </button>
        </div>

        <ul className={`${showLinks ? 'block': 'hidden'} relative lg:flex justify-center col-span-2 lg:col-span-1 animate-fadeDown`}>
          {navLinks.map(link => (
            <li key={link.id}>
